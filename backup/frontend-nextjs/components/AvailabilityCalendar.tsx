...
const AvailabilityCalendar: React.FC<AvailabilityCalendarProps> = ({
  ...
  const generateTimeSlots = (date: Date, resourceId: string): TimeSlot[] => {
    const slots: TimeSlot[] = [];
    const startHour = 8; // 8:00 AM
    const endHour = 20; // 8:00 PM
    const dateStr = date.toISOString().split('T')[0];
    
    // Patrones más realistas basados en tipo de recurso y hora
    const isToday = dateStr === new Date().toISOString().split('T')[0];
    const isParkingResource = resourceId.includes('-P');
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      for (let minutes of [0, 30]) {
        ...
        // Patrones realistas de ocupación
        if (isParkingResource) {
          // Parking: más ocupado en horas pico (8-10am, 6-8pm)
          if ((hour >= 8 && hour < 10) || (hour >= 18 && hour < 20)) {
            available = Math.random() > 0.7; // 30% disponibilidad en horas pico
          } else {
            available = Math.random() > 0.4; // 60% disponibilidad resto del día
          }
        } else {
          // Salas: más ocupadas en horario de oficina (9-12am, 2-5pm)
          if ((hour >= 9 && hour < 12) || (hour >= 14 && hour < 17)) {
            available = Math.random() > 0.6; // 40% disponibilidad en horario peak
          } else {
            available = Math.random() > 0.3; // 70% disponibilidad en horas tranquilas
          }
        }
        
        // Si es hoy y ya pasó la hora, marcar como no disponible
        if (isToday && hour <= currentHour) {
          available = Math.random() > 0.8; // Solo 20% de slots pasados disponibles
        }
        
        // Agregar algunas reservas fake para mostrar ocupación
    ...
    const loadResourcesAndAvailability = async () => {
      try {
        ...
        availabilityResults.forEach(result => {
          // Si no hay slots de la API, generar slots mock realistas
          if (result.slots.length === 0) {
            result.slots = generateTimeSlots(currentDate, result.resourceId);
          }
